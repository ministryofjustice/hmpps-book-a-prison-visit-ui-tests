...
test.describe('Create a booking via public ui', () => {
  ...
  test.beforeEach(async ({ loginPage, homePage }) => {
    ...
    expect(name).toBe(`${prisonerName} at Foston Hall (HMP & YOI)`)
  ...
  test('Book a visit - with 1 visitor and no additional support needed', async ({
    ...
    const contactPrison = await bookingConfirmationPage.getContactPrison()
    expect(contactPrison).toContain('Foston Hall (HMP & YOI)')
    expect(contactPrison).toContain('0123 456 7890')
